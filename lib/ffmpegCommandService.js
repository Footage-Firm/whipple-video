...
class FfmpegCommandService {
    ...
    _buildFilterComplex(command) {
        ...
        if (command.video) {
            command.video.forEach((video, i) => {
                if (video.padding) {
                    filter += ` color=${video.padding.color}:size=${command.output.crop.outputWidth}x${command.output.crop.outputHeight}:d=${video.padding.duration} [v${i}_padding];`;
                }
            });
        }
        ...
        // if (command.output && command.output.crop) {
            const cropWidth = command.output.crop.outputWidth || outWidth;
            const cropHeight = command.output.crop.outputHeight || outHeight;
            const xLoc = command.output.crop.xLoc || 0;
            const yLoc = command.output.crop.yLoc || 0;
        // const cropWidth = outWidth;
        // const cropHeight = outHeight;
        // const xLoc = 0;
        // const yLoc = 0;
        ...
        command.video.forEach((video,i) => {
            ...
            filter += ` scale=${outWidth} :${outHeight} :force_original_aspect_ratio=increase,`;
            // filter += ` scale='if(gt(iw,ih),${outHeight},trunc(oh*a/2)*2)':'if(gt(iw,ih),trunc(ow/a/2)*2,${outWidth})', `;

            filter += ` crop= w=${cropWidth} :h=${cropHeight} :x=(iw-ow)/2 :y=(ih-oh)/2,`;
            ...
            filter += ` pad= width=${outWidth} :height=${outHeight} :x=(ow-iw)/2 :y=(oh-ih)/2 :color=red,`;

            filter += ` setsar=1,`;

            // crop everything to size
            filter += ` crop= w=${cropWidth} :h=${cropHeight} :x=${xLoc} :y=${yLoc},`;
    ...
    /**
     * Scales a number in relation to how the base font size relates to the
     * output height, which ensures font sizes are consistent across various
     * output dimensions.
     */
    _calculateScalableFontSize(size, outputHeight) {
        return (size / BASE_FONT_SIZE) * (outputHeight / BASE_FONT_SIZE);
    }
    ...
    _createFilterForTextOverlay(textOverlay, i, command) {
        ...
        const text = textOverlay.text
            .replace(bashCharsToEscapeRegex, "\\$&")
            // No other replacement for single quote worked
            .replace(/'/g, "\u2019");

        const defaultMargin = this._calculateScalableFontSize(
        ...
        const size = this._calculateScalableFontSize(textOverlay.fontSize, command.output.dimensions.height);
